...
/etc/passwd
Stores user account information steve:x:1001:1001:Steve User:/home/steve:/bin/bash
Fields in /etc/passwd
Username – The name of the user account.
Password Placeholder – Typically x, indicating the password is stored in /etc/shadow.
User ID (UID) – A unique identifier assigned to the user.
Group ID (GID) – The primary group associated with the user.
User Info (Comment Field) – Additional information, such as the full name of the user.
Home Directory – The default directory for the user.
Login Shell – The shell assigned to the user (e.g., /bin/bash).
/etc/shadow
Username
Encrypted Password
Last time password change
...
Fields in /etc/group
Group Name – The name of the group (e.g., avengers).
Password Placeholder – Usually x, meaning the actual password (if any) is stored in /etc/gshadow.
Group ID (GID) – A unique numerical ID for the group.
Group Members – A comma-separated list of users in the group.
Fields in /etc/gshadow
Group Name – The name of the group (e.g., avengers).
Encrypted Password – If a group has a password, it is stored here. (! or * means no password).
Group Administrators – Users who can manage the group.
Group Members – Regular users in the group.
